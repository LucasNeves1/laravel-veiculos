<x-app-layout>
    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <form method="POST" action="{{ route('atualizar_modelo', $modelo->id) }}" class="grid gap-3">
                @csrf
                @method('PUT')
                <label for="name" class="text-white">Nome do modelo</label>
                <input type="text" class="py-2 px-4 border" placeholder="Nome do modelo" name="name" id="name" value="{{ $modelo->name }}">
                
                <label for="brand_id" class="text-white">Marca relacionada</label>
                <select name="brand_id" id="brand_id">
                    @foreach ($brands as $brand)
                        <option value="{{ $brand->id }}" {{ $modelo->brand_id == $brand->id ? 'selected' : '' }}>{{ $brand->name }}</option>
                    @endforeach
                </select>
